...
public class Sprite {
	...
	*/
    ...
    public boolean boundingBoxCollision(Sprite s1)
    {
    	float x = this.getX();
    	float y = this.getY();
    	int xw = this.getWidth();
    	int xh = this.getHeight();
    	return ((x+xw >= s1.getX()) && (x <= s1.getX()+s1.getWidth()) &&
    			(y+xh >= s1.getY()) && (y <= s1.getY()+s1.getHeight()));
    }
    public boolean boundingCircleCollision(Sprite s1)
    {
    	float dx, dy, min;
    	dx = s1.getX() - this.getX();
    	dy = s1.getY() - this.getY();
    	min = s1.getRadius() - this.getRadius();
